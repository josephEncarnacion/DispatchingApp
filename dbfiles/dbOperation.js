...
async function insertUser(user) {
    try {
        let pool = await sql.connect(config);
        const query = 'INSERT INTO users (username, first_name, last_name, password, role) VALUES (@username, @firstName, @lastName, @password, @role)';
        await pool.request()
...
const deleteComplaintByName = async (name) => {
    try {
        let pool = await sql.connect(config);
         // Retrieve complaint and user details before deleting
        ...
        await pool.request()
            ...
            if (complaintResult.recordset.length > 0) {
                ...
                const Message = 'Your complaint report has been deleted.';
    ...
    try {
        ...
        await pool.request()
            ...
            if (emergencyResult.recordset.length > 0) {
                ...
                const Message = 'Your emergency report has been deleted.';
        ...
        if (complaintResult.recordset.length > 0) {
            ...
            await pool.request()
             ...
             .query(`
                 ...
                 VALUES (@userId, @message)
             `);
         // Add notification for the Response team base on the 2012 user
            const responseTeamUserIds = [2, 4];
            const notificationMessage = 'New confirmed Complaint ready for dispatch';

         for (const responseUserId of responseTeamUserIds) {
             await pool.request()
                 .input('userId', sql.Int, responseUserId)
                 .input('message', sql.VarChar, notificationMessage)
                 .query(`
                     INSERT INTO Notifications (user_id, message) 
                     VALUES (@userId, @message)
                 `);
...
const confirmEmergencyByName = async (name) => {
    try {
        ...
        if (emergencyResult.recordset.length > 0) {
            ...
            // Add notification for the Response team base on the 2012 user
            const responseTeamUserIds = [2, 4];
            const notificationMessage = 'New confirmed emergency ready for dispatch';
